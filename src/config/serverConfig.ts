  ...
  transport: new WebSocketTransport({
    ...
    verifyClient: (info, callback) => {
      ...
      if (!origin || allowedOrigins.includes(origin)) {
        console.log(
          "[Colyseus] ✅ WebSocket connection allowed | origin:",
          origin,
        );
        ...
        console.log(
          "[Colyseus] ❌ WebSocket connection rejected from origin:",
          origin,
        );
        // eslint-disable-next-line node/no-callback-literal
...
colyseusServer.define("chess_room", ChessRoom as any).filterBy(["gameId"]);
